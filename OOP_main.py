...
class SCORE(World):
    ...
    def score_display(self, game_state):
        ...
        if game_state == 'main game':
            score_surface = self.game_font.render(str(int(self.current_score)), True, font_color)
            score_rect = score_surface.get_rect(center=(x_score, y_score))
            screen.blit(score_surface, score_rect)
        if game_state == 'game over':
            score_surface = self.game_font.render(f"High Score: {str(int(self.high_score))}", True, font_color)
            score_rect = score_surface.get_rect(center=(x_high_score, y_high_score))
            screen.blit(score_surface, score_rect)

            high_score_surface = self.game_font.render(str(int(self.current_score)), True, font_color)
